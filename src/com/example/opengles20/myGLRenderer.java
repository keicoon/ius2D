...
public class myGLRenderer implements GLSurfaceView.Renderer {
    ...
    private FrameCounter mFrameCounter;			// Frame을 측정하기 위한 클래스
	...
	public myGLRenderer(Context context, myGLSurfaceView gl20) {
		mGL20 = gl20;
		mContext = context;
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		final String vertexShaderCode = RawResourceReader.readTextFileFromRawResource(mContext, R.raw.object_vertex_shader);
		final String fragmentShaderCode = RawResourceReader.readTextFileFromRawResource(mContext, R.raw.object_fragment_shader);
		final int vertexShaderHandle = ShaderHelper.compileShader(GLES20.GL_VERTEX_SHADER, vertexShaderCode);		
		final int fragmentShaderHandle = ShaderHelper.compileShader(GLES20.GL_FRAGMENT_SHADER, fragmentShaderCode);		
		mObjectProgramHandle = ShaderHelper.createAndLinkProgram(vertexShaderHandle, fragmentShaderHandle, 
				new String[] {"a_Position",  "a_Color", "a_TexCoordinate"});
		
		final String pointVertexShader = RawResourceReader.readTextFileFromRawResource(mContext, R.raw.font_vertex_shader);        	       
		final String pointFragmentShader = RawResourceReader.readTextFileFromRawResource(mContext, R.raw.font_fragment_shader);    
		final int pointVertexShaderHandle = ShaderHelper.compileShader(GLES20.GL_VERTEX_SHADER, pointVertexShader);
		final int pointFragmentShaderHandle = ShaderHelper.compileShader(GLES20.GL_FRAGMENT_SHADER, pointFragmentShader);
        mFontProgramHandle = ShaderHelper.createAndLinkProgram(pointVertexShaderHandle, pointFragmentShaderHandle, 
        		new String[] {"a_Position",  "a_Color", "a_TexCoordinate"});
        
        final String bwVertexShader = RawResourceReader.readTextFileFromRawResource(mContext, R.raw.bw_vertex_shader);        	       
        final String bwFragmentShader = RawResourceReader.readTextFileFromRawResource(mContext, R.raw.bw_fragment_shader);    
		final int bwVertexShaderHandle = ShaderHelper.compileShader(GLES20.GL_VERTEX_SHADER, bwVertexShader);
		final int bwFragmentShaderHandle = ShaderHelper.compileShader(GLES20.GL_FRAGMENT_SHADER, bwFragmentShader);
	...
	/* 빛을 그리는 함수 < 일반적으로 사용하지 않아 주석으로 함 >
	private void drawLight(int mPointProgramHandle)
	{
